...
//Rare exceptions to indicate a bug in the _bayapos code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_bayapos = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_bayapos]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.bayapos.module"
